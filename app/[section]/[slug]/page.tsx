...
import {getAllPageRoutes, getHomePage, getPageByRoute, getPostsByTags, getSiteSettings} from "@/sanity/lib/queries";
...
export default async function SectionPage({params}: {params: Promise<{section: string; slug: string}>}) {
  ...
  const page = await getPageByRoute(section, slug);
  if (!page) notFound();
  const [homePage, siteSettings, relatedPosts] = await Promise.all([
    getHomePage(),
    getSiteSettings(),
    getPostsByTags(page.sidebarNewsTags || [])
  ]);

  return <SiteShell><CmsPage page={page} homePage={homePage} siteSettings={siteSettings} relatedPosts={relatedPosts} /></SiteShell>;
}
